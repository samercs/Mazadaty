...
$gitSubject = git.exe log -1 --pretty=format:%s
$gitName = git.exe log -1 --pretty=format:%cn
$gitEmail = git.exe log -1 --pretty=format:%ce

$title = "Deployment: $siteName";
$fallback = "Deployment of $siteName complete.";
$text = "Completed: $dateTime\nCommit Hash: $gitFullHash\nSubject: $gitSubject\nCommitter: $gitName [$gitEmail]";
...
{
    [CmdletBinding(SupportsShouldProcess=$false, DefaultParameterSetName='SeverityOrColour')]
    ...
    (
        ...
        [Parameter(Mandatory=$false, ParameterSetName='SeverityOrColour')]
        [ValidateSet("good", "warning", "danger")]
        [String]
        $Severity,
        
        [Parameter(Mandatory=$false, ParameterSetName='ColourOrSeverity')]
        [Alias("Colour")]
        [string]
        $Color,
...
New-SlackRichNotification -Title $title -Text $text -Fallback $fallback -Severity good -UserName "Git Deployment" | Send-SlackNotification -Url "https://hooks.slack.com/services/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" 